    ...
    try {
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                ///////// File that can save cordnate and time 
                
                struct timeb timebuffer;
                ftime(&timebuffer);
                printf("%lld.%03d \n", timebuffer.time, timebuffer.millitm);
                FILE *fp = fopen("./coordinate_and_time.txt", "at");
                fprintf(fp, "%s %d %d %d %d %lld.%03d \n", names[class_id], left, top, right, bot, timebuffer.time, timebuffer.millitm);
                fclose(fp);
